...
sub vcl_recv {
  ...
  if (req.url ~ "(\/{{ parent().get_service_by_role('varnish').environment.CPTRIGGER }}|p={{ parent().get_service_by_role('varnish').environment.CPTRIGGER }})(.*)" || req.url ~ "^/actions(.*)" || req.url ~ "^/index.php/actions(.*)") {
    return(pass);
  }
